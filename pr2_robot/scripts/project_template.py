...
def pcl_callback(pcl_msg):
    ...
    # Set threshold for number of std. dev.
    x = 0.5
...
def pr2_mover(object_list):
    ...
    # Read through the pick list in file
    pick_list_name = []
    object_group = []
    num_objects = len(object_list_param)
    for i in range(num_objects):
	pick_list_name.append(object_list_param[i]['name'])
	object_group.append(object_list_param[i]['group'])

    # TODO: Rotate PR2 in place to capture side tables for the collision map
    # Read through the dropbox info in file
    dropbox_name = []
    dropbox_group = []
    dropbox_position = []
    num_dropboxes = len(dropbox_param)
    for i in range(num_dropboxes):
	...
	object_name.data = label
        ...
        # Check if the object is in the target list
	if any(s==label for s in pick_list_name):
	    # Get the group of the detected object
	    whichgroup = object_group[pick_list_name.index(label)]
	else:
            # Randomly guess a side to place
	    whichgroup = dropbox_group[np.random.randint(0,2)]
	    
	
	# Obtain centroid
	points_arr = ros_to_pcl(object.cloud).to_array()
	centroid = np.mean(points_arr, axis=0)[:3]
	centroid_scalar = [np.asscalar(x) for x in centroid]

        # TODO: Create 'place_pose' for the object
	# Create pick position
	pick_pose = Pose()
	pick_pose.position.x = centroid_scalar[0]
	pick_pose.position.y = centroid_scalar[1]
	pick_pose.position.z = centroid_scalar[2]

	# Create place position
	place_pose = Pose()
	pos = dropbox_position[dropbox_group.index(whichgroup)]
	...
	arm_name = String()
	arm_name.data = dropbox_name[dropbox_group.index(whichgroup)]
    ...
    filename = "output_{}.yaml".format(test_scene_num)
...
if __name__ == '__main__':
    ...
    detected_objects_pub = rospy.Publisher("/detected_object", DetectedObjectsArray, queue_size=1)
